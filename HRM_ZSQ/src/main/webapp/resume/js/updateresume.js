...
function  updateResumetijiao() {
    ...
    if(rNameVerify(rName)&&rBirthdateVerify(rBirthdate)&&XbjPmpEEbraunZHVerify()
        ...
        var dataResume ="rId="+rId+"&rName="+rName+"&rSex="+sSex+"&rBirthdate="+rBirthdate+
        ...
        $.ajax({
            type:"post",
            data:dataResume,
            url:"ResumeController/updateByUidAndRidResume.do",
            success:function (obj) {
                if(obj=='ok'){
                    alert("简历修改成功！");
                    location.reload();
                }else{
                    alert("简历修改失败！请检查信息是否填写完整！")
                }
            }
        });
    }
